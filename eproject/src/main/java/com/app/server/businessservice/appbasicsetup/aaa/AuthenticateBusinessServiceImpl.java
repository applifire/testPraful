		...
		try {
			...
			if (appConfig.getAuthenticationConfig().getAuthPlugin().getAuthType() == 1) {
				...
				if (userBean != null) {
					...
					if (userData.getTotalNumberOfPassRecovery() == 0 && !(userData.getUserId().equals("18D01ABF-F632-496A-B379-FC50EDEAB8C0") || userData.getUserId().equals("F332F408-BED0-4F0C-BEAA-E007E8FFB21E"))) {
	...
	private void saveSessionData(HttpSession session, HttpServletRequest request, UserBean userBean) throws Exception {
		...
		if (loginSession != null) {
			...
			try {
				...
				if (runtimeLogInfoHelper.getCustomerId() != null && !runtimeLogInfoHelper.getCustomerId().equalsIgnoreCase("system")) {
