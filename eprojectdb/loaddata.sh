...
echo $PATH
OSNAME=`uname -s`
ART_DATA_PATH=/tmp/applifire/db/H5IX8O2HZSWQEQZ92JGHWG/art/data
AST_DATA_PATH=/tmp/applifire/db/H5IX8O2HZSWQEQZ92JGHWG/ast/data
MYSQL=/usr/bin
...
$MYSQL/mysql --local-infile=1 -h$HOST -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "load data local infile '$AST_DATA_PATH/ast_JwtConfig_M.csv' into table ast_JwtConfig_M FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n' (configId, jwtAlgorithm, expiration, tokenKey, createdBy, createdDate, updatedBy, updatedDate, versionId, activeStatus)";
